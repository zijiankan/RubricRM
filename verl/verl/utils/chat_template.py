...
def resolve_continue_final_message(messages: list[dict], apply_kwargs: dict) -> tuple[bool, dict]:
    """Dynamically resolve continue_final_message based on last message role.

    When continue_final_message is configured globally but the last message is
    not from the assistant (e.g. v1 validation data ending with a user message),
    override to add_generation_prompt=True so the template correctly injects the
    assistant turn prefix.  This allows a single global config to work for both
    v1 (user-ending) and v2 (assistant-continuation) prompts.

    Returns:
        (add_generation_prompt, adjusted_apply_kwargs)
    """
    use_continue = apply_kwargs.get("continue_final_message", False)
    if use_continue:
        last_role = messages[-1].get("role", "user") if messages else "user"
        if last_role != "assistant":
            # Strip continue_final_message so the template closes the user turn normally
            apply_kwargs = {k: v for k, v in apply_kwargs.items() if k != "continue_final_message"}
            return True, apply_kwargs
    return not use_continue, apply_kwargs
