            ...
            if processor is not None:
                ...
                def doc2len(doc) -> int:
                    try:
                        ...
                        add_gen, apply_kwargs = resolve_continue_final_message(messages, apply_kwargs)
                        raw_prompt = self.processor.apply_chat_template(
                            messages, add_generation_prompt=add_gen, tokenize=False, **apply_kwargs
                        )
                ...
                def doc2len(doc) -> int:
                    try:
                        ...
                        from verl.utils.chat_template import resolve_continue_final_message
                        _msgs = doc[prompt_key]
                        add_gen, apply_kwargs = resolve_continue_final_message(_msgs, apply_kwargs)
                        tokenized_prompt = tokenizer.apply_chat_template(
                            _msgs, add_generation_prompt=add_gen, tokenize=True, **apply_kwargs
                        )
