...
class TaskRunner:
    ...
    def run(self, config):
        ...
        # Support named validation files for independent evaluation
        named_val_files = config.data.get("named_val_files", None)
        if named_val_files:
            # Each entry is name -> file_path(s); datasets created in _create_dataloader
            val_dataset = None
        else:
