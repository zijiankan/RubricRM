...
class RayPPOTrainer:
    ...
    def _create_dataloader(self, train_dataset, val_dataset, collate_fn, train_sampler: Optional[Sampler]):
        ...
        Supports named_val_files for independent multi-set evaluation.
        """
        ...
        # Build validation datasets: named (independent) or single (backward compat)
        named_val_files = self.config.data.get("named_val_files", None)
        val_max_samples = self.config.data.get("val_max_samples", -1)

        if named_val_files:
            # Multiple named validation sets – each evaluated independently
            val_datasets: dict[str, Dataset] = {}
            for name, path in named_val_files.items():
                val_datasets[name] = create_rl_dataset(
                    path, self.config.data, self.tokenizer, self.processor,
                    is_train=False, max_samples=val_max_samples,
                )
        else:
            # Single validation set (backward compat)
            ...
            val_datasets = {"default": val_dataset}

        # Create a dataloader per validation set
        val_batch_size_cfg = self.config.data.val_batch_size
        self.val_dataloaders: dict[str, StatefulDataLoader] = {}
        for name, ds in val_datasets.items():
            bs = val_batch_size_cfg if val_batch_size_cfg is not None else len(ds)
        ...
        # Backward compat: keep self.val_dataset / self.val_dataloader
        first_name = next(iter(val_datasets))
        self.val_dataset = val_datasets[first_name]
        self.val_dataloader = self.val_dataloaders[first_name]

        assert len(self.train_dataloader) >= 1, "Train dataloader is empty!"
        for vname, vdl in self.val_dataloaders.items():
            assert len(vdl) >= 1, f"Validation dataloader '{vname}' is empty!"

        val_info = ", ".join(f"{n}: {len(dl)}" for n, dl in self.val_dataloaders.items())
        print(
            f"Size of train dataloader: {len(self.train_dataloader)}, "
            f"Val dataloaders: {val_info}"
        )
    ...
    def _validate(self, merged: bool = False):
        """Run validation on all configured validation sets."""
        has_named = len(self.val_dataloaders) > 1 or "default" not in self.val_dataloaders

        if not has_named:
            # Single default set – backward compat (no metric prefix)
            return self._validate_single(self.val_dataloaders["default"], merged=merged)

        # Multiple named sets – evaluate each independently
        all_metrics: dict = {}
        for set_name, dataloader in self.val_dataloaders.items():
            print(f"\n=== Validation set: {set_name} ===")
            result = self._validate_single(dataloader, set_name=set_name, merged=merged)
            if merged:
                # merged mode returns raw dicts; keep per-set for callers
                all_metrics[set_name] = result
            else:
                all_metrics.update(result)

        if merged:
            # Merge across named sets is non-trivial; return combined raw results
            combined = {"data_sources": [], "sample_uids": [], "sample_turns": [], "reward_extra_infos_dict": {}}
            for res in all_metrics.values():
                if res:
                    combined["data_sources"].extend(res.get("data_sources", []))
                    combined["sample_uids"].extend(res.get("sample_uids", []))
                    combined["sample_turns"].extend(res.get("sample_turns", []))
                    for k, v in res.get("reward_extra_infos_dict", {}).items():
                        combined["reward_extra_infos_dict"].setdefault(k, []).extend(v)
            return combined
        return all_metrics

    def _validate_single(self, dataloader, set_name: str = "", merged: bool = False):
        data_source_lst = []
        ...
        if val_data_dir:
            if set_name:
                dump_path = os.path.join(val_data_dir, set_name)
            else:
        ...
        return self._val_metrics_update(data_sources, sample_uids, reward_extra_infos_dict, sample_turns, set_name=set_name)

    def _val_metrics_update(self, data_sources, sample_uids, reward_extra_infos_dict, sample_turns, set_name: str = ""):
            ...
            for var_name, metric2val in var2metric2val.items():
                ...
                for metric_name, metric_val in metric2val.items():
                    if (
                        ...
                        metric_sec = f"val_{set_name}-core" if set_name else "val-core"
                    else:
                        metric_sec = f"val_{set_name}-aux" if set_name else "val-aux"
                    pfx = f"{metric_sec}/{data_source}/{var_name}/{metric_name}"
                    metric_dict[pfx] = metric_val

        if len(sample_turns) > 0:
            sample_turns = np.concatenate(sample_turns)
            turns_pfx = f"val_{set_name}-aux" if set_name else "val-aux"
            metric_dict[f"{turns_pfx}/num_turns/min"] = sample_turns.min()
            metric_dict[f"{turns_pfx}/num_turns/max"] = sample_turns.max()
            metric_dict[f"{turns_pfx}/num_turns/mean"] = sample_turns.mean()
    ...
    def fit(self):
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # V2 format parse failure ratio (from reward function)
                if "v2_parse_failed" in batch.non_tensor_batch:
                    vals = np.asarray(batch.non_tensor_batch["v2_parse_failed"], dtype=np.float32)
                    metrics["reward/v2_parse_failed_ratio"] = float(np.mean(vals))
                # Reward saturation filter ratio (groups with all-similar rewards skipped)
                saturation_std_threshold = self.config.algorithm.get("grpo_reward_saturation_std", None)
                if saturation_std_threshold is not None and "uid" in batch.non_tensor_batch:
                    rewards_sum = batch.batch["token_level_rewards"].sum(dim=-1)
                    uids = batch.non_tensor_batch["uid"]
                    uid2rewards = defaultdict(list)
                    for i, uid in enumerate(uids):
                        uid2rewards[uid].append(rewards_sum[i].item())
                    total_groups = sum(1 for r in uid2rewards.values() if len(r) > 1)
                    filtered_groups = sum(
                        1 for r in uid2rewards.values()
                        if len(r) > 1 and float(np.std(r)) < saturation_std_threshold
                    )
                    if total_groups > 0:
                        metrics["reward/saturation_filtered_ratio"] = filtered_groups / total_groups
