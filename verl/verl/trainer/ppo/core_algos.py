...
def compute_grpo_outcome_advantage(
    ...
    # Get reward saturation filter threshold from config
    saturation_std = None
    if config is not None:
        saturation_std = config.get("grpo_reward_saturation_std", None)
    ...
    with torch.no_grad():
        ...
        for idx in id2score:
            ...
            # Filter reward-saturated groups (all rollouts have similar rewards)
            if saturation_std is not None and len(id2score[idx]) > 1 and id2std[idx].item() < saturation_std:
                filtered_ids.add(idx)

        for i in range(bsz):
            if index[i] in filtered_ids:
                scores[i] = 0.0
    ...
    # Log filtering stats
    if filtered_ids:
        total_groups = len([idx for idx in id2score if len(id2score[idx]) > 1])
        logger.info(
            f"[GRPO] Reward saturation filter: {len(filtered_ids)}/{total_groups} groups filtered "
            f"(std < {saturation_std})"
        )
